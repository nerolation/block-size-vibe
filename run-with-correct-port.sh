...
# Start the backend with specified endpoints
echo "Starting backend with local endpoints..."
cd backend
# Use environment variables to explicitly set the URLs
BEACON_NODE_URL=http://localhost:5052/ EXECUTION_NODE_URL=http://localhost:8545/ $PYTHON_CMD app.py &
BACKEND_PID=$!
cd ..
