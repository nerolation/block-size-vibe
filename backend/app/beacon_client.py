...
def get_execution_node_url():
    ...
    return 'https://example.com/execution/'
...
def get_execution_rpc_url():
    """Get the execution RPC URL from the current app configuration or environment"""
    if current_app and current_app.config.get('EXECUTION_NODE_URL'):
        return current_app.config['EXECUTION_NODE_URL']
    elif os.getenv('EXECUTION_NODE_URL'):
        return os.getenv('EXECUTION_NODE_URL')
    else:
        return 'https://example.com/execution/' 